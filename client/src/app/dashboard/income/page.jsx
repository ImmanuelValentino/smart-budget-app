...
const IncomePage = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            setCategoryOptions(catRes.data.pemasukan.map(cat => ({ value: cat, label: cat })));
    ...
    const handleCreateCategory = async (inputValue) => {
        try {
            const { data } = await api.post('/categories', { name: inputValue, type: 'pemasukan' });
            const newCategory = { value: data.name, label: data.name };
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="md:col-span-1 bg-gray-800 p-6 rounded-lg">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            {isClient ? (
                                <ClientCreatableSelect
                                    ...
                                    value={form.category}
                                    onChange={(newValue) => setForm(prev => ({ ...prev, category: newValue }))}
                        ...
                        {editingId && (
                            <button type="button" onClick={resetForm} className="w-full mt-2 py-2 bg-gray-600 hover:bg-gray-500 rounded-md font-bold">
                                Batal
                            </button>
                        )}
                    </form>
                        ...
                        {transactions.map((trans) => (
                            <div key={trans._id} className="flex justify-between items-center bg-gray-700 p-3 rounded">
                                ...
                                <div className="flex items-center space-x-4">
                                    <p className="font-bold text-green-400">+ Rp {trans.amount.toLocaleString('id-ID')}</p>
